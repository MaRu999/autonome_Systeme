...
for j=1:field_num
    for i=1:field_num
        for k=1:max(size(obstacle_mids))
            u_reps(i,j) = u_reps(i,j) + u_rep(k_rep, roh_min, [i;j], obstacle_mids(:,k), obstacle_radii(k), scaling_factor);
        end
    end
end
% scaling_factor = 1;
...
hold on;
scaling_factor = 1;
vec_x = ([6, 6] + octave_offset) * scaling_factor;
vec_y = ([4, 4] + octave_offset) * scaling_factor;
vec_z = [0, up_lim];
plot3(vec_x, vec_y, vec_z);
vec_x = ([7, 7] + octave_offset) * scaling_factor;
vec_y = ([8, 8] + octave_offset) * scaling_factor;
vec_z = [0, up_lim];
plot3(vec_x, vec_y, vec_z);
vec_x = ([3, 3] + octave_offset) * scaling_factor;
vec_y = ([2.2, 2.2] + octave_offset) * scaling_factor;
vec_z = [0, up_lim];
plot3(vec_x, vec_y, vec_z);
hold off;
axis("ij");